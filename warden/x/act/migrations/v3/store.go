...
func migrate(ctx sdk.Context, ns NewStore, os OldStore) error {
	...
	for ; actsByAddress.Valid(); actsByAddress.Next() {
		actsByAddress, err := actsByAddress.Key()
		if err != nil {
			return err
		}
		ns.ActionByAddress.Set(ctx, actsByAddress)
	}
...
type NewStore struct {
	...
	ActionByAddress collections.KeySet[collections.Pair[sdk.AccAddress, uint64]]
}
...
func newStore(storeService store.KVStoreService, cdc codec.BinaryCodec) (NewStore, error) {
	...
	actionByAddress := collections.NewKeySet(sb, ActionByAddressPrefix, "action_by_address", collections.PairKeyCodec(sdk.AccAddressKey, collections.Uint64Key))
