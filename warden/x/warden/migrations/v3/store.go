...
func migrate(ctx sdk.Context, ns NewStore, os OldStore) error {
	...
	if err := migrateSpacesByOwner(ctx, ns, os, ownersMap); err != nil {
		return err
	}
	...
	for ; keysBySpace.Valid(); keysBySpace.Next() {
		...
		ns.KeysBySpace.Set(ctx, collections.Join(keyBySpace.K1(), keyBySpace.K2()))
	}
...
func migrateSpacesByOwner(ctx sdk.Context, ns NewStore, os OldStore, ownersMap map[string][]uint64) error {
